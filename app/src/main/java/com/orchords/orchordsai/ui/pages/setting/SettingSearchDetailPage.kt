...
import com.orchords.search.DoubaoSearchMode
import com.orchords.search.SearchCommonOptions
import com.orchords.search.SearchResult
...
private fun SearchServiceOptionsEditor(
    ...
    when (options) {
        ...
        is SearchServiceOptions.OrchordsAIOptions -> {
            OrchordsAIOptions(options) { onUpdateOptions(it) }
        }
...
@Composable
internal fun OrchordsAIOptions(
    options: SearchServiceOptions.OrchordsAIOptions,
    onUpdateOptions: (SearchServiceOptions.OrchordsAIOptions) -> Unit
) {
    FormItem(
        label = {
            Text(stringResource(R.string.search_detail_api_key))
    ...
    FormItem(
        label = {
            Text(stringResource(R.string.search_detail_base_url))
        }
    ) {
        OutlinedTextField(
            ...
            placeholder = { Text("https://api.example.com/v1/search") },
            modifier = Modifier.fillMaxWidth()
        )
    }
