...
- [ ] **OSV scan operational (deeeedc):** the Security Analysis OSV Dependency Cross-check job now installs `osv-scanner v2.5.1` from the official `osv-scanner_linux_amd64` release asset (no Go toolchain required) and runs `scan source -r .` successfully on run 34777295551. The job is wired to fail on any OSV finding, so it surfaces two real vulnerabilities in `web-ui/pnpm-lock.yaml`: `js-yaml 4.3.1` (fix: `4.3.2`, CVE GHSA-8qvm-5x2c-j2rf) and `morgan 1.11.0` (fix: `1.12.0`, CVE GHSA-6x3m-p74w-p85c). These are pinned transitive dependencies and must be fixed by bumping the lockfile (or `pnpm.overrides`); the upgrade is a separate agenda item, not a workflow defect.
