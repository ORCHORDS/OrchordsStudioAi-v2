...
**Prepared:** 2026-09-14
**HEAD evidence captured on:** `git rev 16bce6c` (2026-09-14)
...
- [ ] **Deletion-path credential wipe (Pending):** `ProviderSecretCodec.removeDroppedProviders` is not present on `main`; an earlier cycle drafted it together with two regression tests, but the change did not compile under the Gradle daemon and was reverted. Re-evaluating whether this fix is needed for Play submission: the privacy policy's "Delete provider credentials" path only routes through the Settings UI button; the existing path calls `ProviderSecretBackend.remove(providerId)` indirectly via `SettingsStore.update` and does not currently cross-check the encrypted store. The remaining work for PLAY submission is to land the function under test, not to rebrand the existing story.
