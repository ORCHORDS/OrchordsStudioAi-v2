...
class RootfsPatcher {
    ...
    private fun ensureRootfsDns(
        ...
        val approvedServers = nameservers
            .map { it.trim() }
            .filter { it.isNotBlank() && it !in LOCAL_RESOLVERS }
            .distinct()
            .take(MAX_DNS_SERVERS)

        // No approved Android/Workspace resolver policy was supplied. Preserve the archive's
        // resolver state (including local stubs) rather than silently downgrading to public DNS.
        // Network-aware callers can provide the active Android network's resolver set explicitly.
        if (approvedServers.isEmpty()) return

        val resolvConf = File(etcDir, "resolv.conf")
        val currentServers = if (resolvConf.isFile && !Files.isSymbolicLink(resolvConf.toPath())) {
            resolvConf.readText()
                .lineSequence()
                .filter { it.trimStart().startsWith("nameserver ") }
                .map { line -> line.trim().removePrefix("nameserver").trim() }
                .filter { it.isNotBlank() }
                .toList()
        } else {
            emptyList()
        }
        if (currentServers == approvedServers) return
        ...
        resolvConf.writeText(
            buildString {
                appendLine("# Generated by ORCHORDS AI workspace from approved network policy.")
                approvedServers.forEach { appendLine("nameserver $it") }
