...
class ExampleUnitTest {
    ...
    fun rootfsInstallerDownloadsAndExtractsTarGz() {
        assumeFalse("Rootfs extraction requires Unix file modes and symlinks", System.getProperty("os.name").orEmpty().startsWith("Windows", ignoreCase = true))
        val baseDir = Files.createTempDirectory("workspace-manager-test").toFile()
        val manager = WorkspaceManager(baseDir)
    ...
    fun rootfsPatcherAppliesAndroidProotDefaults() {
        ...
        assertEquals(
            """
            # Generated by ORCHORDS AI workspace from approved network policy.
            nameserver 9.9.9.9
            nameserver 8.8.8.8
